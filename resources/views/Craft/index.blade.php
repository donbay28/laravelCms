@include('template/header')
    <h1 class="text-3xl text-black pb-6">Master Craft</h1>
    <a href="{{ url('/craft/create_craft')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 border border-blue-700 rounded">Create</a>
            ...
            <table class='mx-auto max-w-5xl w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 overflow-hidden'>
                <thead class="bg-lightblue-900">
                    <tr class="text-black text-center">
                        <th class="font-semibold text-sm uppercase px-6 py-4"> Nama Craft </th>
                ...
                <tbody class="divide-y divide-gray-200 text-center">
                    @if($craft ?? '' != null)
                        @foreach($craft ?? '' ?? '' as $row)
                            <tr>
                                <td>{{$row->namaCraft}}</td>
                                <td>{{$row->shortDescriptionCraft}}</td>
                                <td>{{$row->descriptionCraft}}</td>
                                <td><img src="{{ url('assets/img/' . $row->imageSmallCraft) }}" style="width: 100px; height:100px;"></td>
                                <td><img src="{{ url('assets/img/' . $row->imageBigCraft) }}" style="width: 100px; height:100px;"></td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="{{url('craft/update_craft/'.$row->idcrafts)}}">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="#" class="delete-culture" craftName="{{$row->namaCraft}}" id="{{$row->idcrafts}}">
                                    ...
                                    <input type="hidden" id="idcrafts" value="{{$row->idcrafts}}">
                                </td>
                            </tr>
                        @endforeach
                    @else
    ...
    <script>
        $(document).on("click", ".delete-culture", function(e) {
            ...
            let namaCraft = $(this).attr('craftName');
            $('#title').text("Delete Data " + namaCraft)
            $('#deleteId').val(del_id)
        });
