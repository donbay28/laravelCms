@include('template/header')
    <h1 class="text-3xl text-black pb-6">Create Culinary</h1>
    ...
    <div class="px-8 py-6 mt-4 text-left bg-white shadow-lg">
        ...
        @endif
        <h3 class="text-2xl font-bold text-center">Input Data Culinary</h3>
        <form id="loginform" action="{{ url('/culinary/create')}}" method="post"  enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="mt-4">
                <div>
                    <label class="block" for="Culinary">Pilih Kota<label>
                    ...
                    @foreach($kota as $val)
                    <option value="{{$val->idkotas}}">{{$val->namaKota}}</option>
                    @endforeach
                    </select>
                </div>
                <div>
                    <label class="block" for="Culinary">Nama Culinary<label>
                    <input type="text" name="culinary" placeholder="Culinary" class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600">
                </div>
                <div>
                    <label class="block">Short Description<label>
                    <textarea type="text" name="shortDescriptionCulinary" placeholder="Short Description" class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600"></textarea>
                </div>
                <div>
                    <label class="block">Description<label>
                    <textarea type="text" name="descriptionCulinary" placeholder="Description" class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600"></textarea>
                </div>
