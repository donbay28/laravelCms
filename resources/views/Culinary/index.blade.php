@include('template/header')
    <h1 class="text-3xl text-black pb-6">Master Culinary</h1>
    <a href="{{ url('/culinary/create_culinary')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 border border-blue-700 rounded">Create</a>
            ...
            <table class='mx-auto max-w-5xl w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 overflow-hidden'>
                <thead class="bg-lightblue-900">
                    <tr class="text-black text-center">
                        <th class="font-semibold text-sm uppercase px-6 py-4"> Nama Culinary </th>
                ...
                <tbody class="divide-y divide-gray-200 text-center">
                    @if($culinary ?? '' != null)
                        @foreach($culinary ?? '' ?? '' as $row)
                            <tr>
                                <td>{{$row->namaCulinary}}</td>
                                <td>{{$row->shortDescriptionCulinary}}</td>
                                <td>{{$row->descriptionCulinary}}</td>
                                <td><img src="{{ url('assets/img/' . $row->imageSmallCulinary) }}" style="width: 100px; height:100px;"></td>
                                <td><img src="{{ url('assets/img/' . $row->imageBigCulinary) }}" style="width: 100px; height:100px;"></td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="{{url('culinary/update_culinary/'.$row->idculinary)}}">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="#" class="delete-Culinaries" CulinariesName="{{$row->namaCulinaries}}" id="{{$row->idculinary}}">
                                    ...
                                    <input type="hidden" id="idCulinariess" value="{{$row->idculinary}}">
                                </td>
                            </tr>
                        @endforeach
                    @else
    ...
    <script>
        $(document).on("click", ".delete-Culinaries", function(e) {
            $('#myModal').show();
            let del_id = $(this).attr('id');
            let namaCulinaries = $(this).attr('CulinariesName');
            $('#title').text("Delete Data " + namaCulinaries)
            $('#deleteId').val(del_id)
        });
