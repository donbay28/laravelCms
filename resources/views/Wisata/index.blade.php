@include('template/header')
    <h1 class="text-3xl text-black pb-6">Master Wisata</h1>
    <a href="{{ url('/wisata/create_wisata')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 border border-blue-700 rounded">Create</a>
                    ...
                    @if($wisata ?? '' != null)
                        @foreach($wisata ?? '' ?? '' as $row)
                            <tr>
                                ...
                                <td>{{$row->shortDescriptionWisata}}</td>
                                <td>{{$row->descriptionWisata}}</td>
                                <td><img src="{{ url('assets/img/' . $row->imageSmallWisata) }}" style="width: 100px; height:100px;"></td>
                                <td><img src="{{ url('assets/img/' . $row->imageBigWisata) }}" style="width: 100px; height:100px;"></td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="{{url('wisata/update_wisata/'.$row->idwisatas)}}">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="#" class="delete-wisata" wisataName="{{$row->namaWisata}}" id="{{$row->idwisatas}}">
                                    ...
                                    <input type="hidden" id="idwisatas" value="{{$row->idwisatas}}">
                                </td>
                            </tr>
                        @endforeach
                    @else
    ...
    <script>
        $(document).on("click", ".delete-wisata", function(e) {
            $('#myModal').show();
            let del_id = $(this).attr('id');
            let namaWisata = $(this).attr('wisataName');
            $('#title').text("Delete Data " + namaWisata)
            $('#deleteId').val(del_id)
        });
