@include('template/header')
    ...
    <a href="{{ url('/kota/create_kota')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 border border-blue-700 rounded">Create</a>
            ...
            <table class='mx-auto max-w-5xl w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 overflow-hidden'>
                <thead class="bg-lightblue-900">
                    <tr class="text-black text-center">
                        <th class="font-semibold text-sm uppercase px-6 py-4"> Nama Kota </th>
                    ...
                    @if($kota ?? '' != null)
                        @foreach($kota ?? '' ?? '' as $row)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="#" class="delete-kota" kotaName="{{$row->namaKota}}" id="{{$row->idkotas}}">
                                    ...
                                    <input type="hidden" id="idkotas" value="{{$row->idkotas}}">
                                </td>
                            </tr>
                        @endforeach
                    @else
    ...
    <script>
        $(document).on("click", ".delete-kota", function(e) {
            $('#myModal').show();
            let del_id = $(this).attr('id');
            let namaKota = $(this).attr('kotaName');
            $('#title').text("Delete Data " + namaKota)
            $('#deleteId').val(del_id)
        });
